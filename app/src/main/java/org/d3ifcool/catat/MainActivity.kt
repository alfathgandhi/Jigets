...
class MainActivity : AppCompatActivity(), GantiPassDialog.DialogListener, TambahTypeDialog.DialogListener2 ,TambahPartDialog.DialogListener3, Communicator {
        ...
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.LOLLIPOP) {
            test.setOnMenuItemClickListener { item ->
                if (item.itemId == R.id.bt_logout) {
                    ...
                    startActivity(Intent(this@MainActivity, WebView_Acitivity::class.java))
                }
                false
            }
        }
    ...
    override fun Lister2(list: List<String>){
        ...
        uploadImageAndSaveUri(list[0], 1, null, null, null, null, null)
    }
    ...
    private fun uploadImageAndSaveUri(namaobj: String?, indicator: Int?, plane: String?, basic: String?, keterangan: String?, line: String?, namaType: List<String>?) {
        if(imageUri!=null)
        {
            ...
            uploadTask.continueWithTask(com.google.android.gms.tasks.Continuation<UploadTask.TaskSnapshot, Task<Uri>> { task ->
                ...
                if(task.isSuccessful){
                    ...
                    if(indicator==1) {
                        ...
                        databas.child("Part").push().setValue(Part(namaobj, plane, line, basic, url, namauser, keterangan, namaType))
                    }
    ...
    override fun Lister3(list: List<String>?, arrayType: List<String>?) {
        ...
        uploadImageAndSaveUri(list?.get(2), 2, list?.get(0), list?.get(3), list?.get(4), list?.get(5), arrayType)
    ...
    override fun sendData(send: String, plane: String, basic: String, line: String, keterangan: String, photo: String, dibuat: List<String>,namaUser:String) {
        val bundle = Bundle()

        bundle.putStringArray("Part", arrayOf(send, plane, basic, line, keterangan, photo, namaUser))
